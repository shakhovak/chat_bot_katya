...
На основе обработанных данных готовятся данные для обучения bi-encoder. Так как обучать я решила c использование triplet loss, то данные преобразуются в триплеты:
...
Посмотреть ноутбук с обученем модел можно [здесь]() . Модель загружена в мой репозиторий на Hugging Face ([ссылка](https://huggingface.co/Shakhovak/chatbot_sentence-transformer)) и уже оттуда будет использоваться в инференсе.
